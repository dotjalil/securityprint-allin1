...
const ContactByEmailButton = ({ position, toId, toRef, duration }) => {
  ...
  return (
    <div
      className={`flex items-start gap-[27px] mt-[40px] flex-col sm:flex-row ${
        position === "right" ? "justify-start" : "justify-center"
      }`}
    >
      <button
        onClick={handleClick}
        className="flex justify-center items-center gap-1.5 text-left uppercase text-white w-full sm:w-auto gap-2.5 px-[26px] py-[12px] rounded-xl"
        style={{
          background:
